  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(pr)) {
        ...
        _push(`</div></div><div class="grid lg:grid-cols-3 gap-6"><div class="lg:col-span-2 space-y-6"><div class="grid md:grid-cols-3 gap-4"><div class="bg-card border border-border rounded-xl p-4"><div class="text-sm text-muted-foreground">Total Amount</div><div class="text-2xl font-bold">₹${ssrInterpolate(unref(pr).totalEstimatedAmount?.toLocaleString())}</div></div><div class="bg-card border border-border rounded-xl p-4"><div class="text-sm text-muted-foreground">Items</div><div class="text-2xl font-bold">${ssrInterpolate(unref(pr).items?.length || 0)}</div></div><div class="bg-card border border-border rounded-xl p-4"><div class="text-sm text-muted-foreground">Required By</div><div class="text-2xl font-bold">${ssrInterpolate(formatDate(unref(pr).required_date))}</div></div></div><div class="bg-card border border-border rounded-xl overflow-hidden"><div class="p-4 border-b border-border"><h3 class="font-semibold">Line Items</h3></div><table class="w-full"><thead class="bg-muted/50"><tr><th class="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Item</th><th class="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Qty</th><th class="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Unit Price</th><th class="px-4 py-3 text-right text-xs font-medium text-muted-foreground uppercase">Total</th></tr></thead><tbody class="divide-y divide-border"><!--[-->`);
        ssrRenderList(unref(pr).items, (item) => {
          _push(`<tr><td class="px-4 py-3"><div class="font-medium">${ssrInterpolate(item.item_name)}</div><div class="text-xs text-muted-foreground">${ssrInterpolate(item.description)}</div></td><td class="px-4 py-3">${ssrInterpolate(item.quantity)} ${ssrInterpolate(item.unit)}</td><td class="px-4 py-3">₹${ssrInterpolate(item.estimated_unit_price?.toLocaleString())}</td><td class="px-4 py-3 text-right font-medium">₹${ssrInterpolate(item.total_estimated_price?.toLocaleString())}</td></tr>`);
        });
        _push(`<!--]--></tbody><tfoot class="bg-muted/30"><tr><td colspan="3" class="px-4 py-3 text-right font-medium">Total Estimated</td><td class="px-4 py-3 text-right text-lg font-bold">₹${ssrInterpolate(unref(pr).totalEstimatedAmount?.toLocaleString())}</td></tr></tfoot></table></div><div class="bg-card border border-border rounded-xl p-6"><h3 class="font-semibold mb-3">Description</h3><p class="text-muted-foreground">${ssrInterpolate(unref(pr).description || "No description provided.")}</p></div><div class="bg-card border border-border rounded-xl p-6"><h3 class="font-semibold mb-4">Activity History</h3><div class="space-y-4"><!--[-->`);
        ...
        _push(`<!--]--></div></div></div><div class="space-y-6"><div class="bg-card border border-border rounded-xl p-6"><h3 class="font-semibold mb-4">Requester</h3><div class="flex items-center gap-3 mb-4"><div class="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center"><span class="font-medium text-primary">${ssrInterpolate(unref(pr).requestedBy?.firstName?.[0])}${ssrInterpolate(unref(pr).requestedBy?.lastName?.[0])}</span></div><div><div class="font-medium">${ssrInterpolate(unref(pr).requestedBy?.firstName)} ${ssrInterpolate(unref(pr).requestedBy?.lastName)}</div><div class="text-sm text-muted-foreground">${ssrInterpolate(unref(pr).requestedBy?.email)}</div></div></div><div class="space-y-2 text-sm"><div class="flex justify-between"><span class="text-muted-foreground">Department</span><span>${ssrInterpolate(unref(pr).department)}</span></div><div class="flex justify-between"><span class="text-muted-foreground">Requested</span><span>${ssrInterpolate(formatDate(unref(pr).created_at))}</span></div></div></div><div class="bg-card border border-border rounded-xl p-6"><h3 class="font-semibold mb-4">Details</h3><div class="space-y-3 text-sm"><div class="flex justify-between"><span class="text-muted-foreground">Category</span><span class="capitalize">${ssrInterpolate(unref(pr).category?.replace("_", " "))}</span></div><div class="flex justify-between"><span class="text-muted-foreground">Priority</span><span class="${ssrRenderClass([getPriorityClass(unref(pr).priority), "px-2 py-0.5 rounded-full text-xs font-medium capitalize"])}">${ssrInterpolate(unref(pr).priority)}</span></div><div class="flex justify-between"><span class="text-muted-foreground">Property</span><span>${ssrInterpolate(unref(pr).property?.name)}</span></div></div></div>`);
        if (unref(pr).approvedBy || unref(pr).approvedAt) {
          ...
          _push(`</div><div><div class="font-medium">${ssrInterpolate(unref(pr).approvedBy?.firstName)} ${ssrInterpolate(unref(pr).approvedBy?.lastName)}</div><div class="text-xs text-muted-foreground">${ssrInterpolate(formatDateTime(unref(pr).approvedAt))}</div></div></div>`);
...
//# sourceMappingURL=index-BJI9SL9W.js.map
