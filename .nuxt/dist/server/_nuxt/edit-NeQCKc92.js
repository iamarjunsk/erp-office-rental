  ...
  async setup(__props) {
    ...
    const filteredSpaces = computed(() => {
      ...
      return (spaces.value || []).filter((space) => {
        return space.property === form.value.property || space.property_details?.id === form.value.property;
      });
    });
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div><h1 class="text-3xl font-bold">Edit Asset</h1><p class="text-muted-foreground">${ssrInterpolate(unref(asset)?.asset_code)}</p></div></div>`);
      if (unref(form)) {
...
//# sourceMappingURL=edit-NeQCKc92.js.map
