...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    if (fromPR.value) {
      ...
      if (prData.value) {
        setTimeout(() => {
          ...
          form.value.items = prData.value.items?.map((item) => ({
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(fromPR)) {
        _push(`<span>Convert PR ${ssrInterpolate(unref(prData)?.prNumber)} to Purchase Order</span>`);
      } else {
...
//# sourceMappingURL=create--Xo47GB6.js.map
