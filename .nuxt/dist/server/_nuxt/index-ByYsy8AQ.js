  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(assets), (asset) => {
        ...
        _push(`<div class="text-xs text-muted-foreground">${ssrInterpolate(asset.asset_code)}</div></div></div></td><td class="px-4 py-4"><div class="text-sm">${ssrInterpolate(asset.category_details?.name || "Uncategorized")}</div></td><td class="px-4 py-4"><div class="text-sm">${ssrInterpolate(asset.property_details?.name)}</div><div class="text-xs text-muted-foreground">${ssrInterpolate(asset.space_details?.code)}</div></td><td class="px-4 py-4">`);
        if (asset.assigned_to_details) {
          _push(`<div class="flex items-center gap-2"><div class="w-7 h-7 rounded-full bg-primary/10 flex items-center justify-center text-xs font-medium text-primary">${ssrInterpolate(asset.assigned_to_details.first_name?.[0])}${ssrInterpolate(asset.assigned_to_details.last_name?.[0])}</div><div><div class="text-sm">${ssrInterpolate(asset.assigned_to_details.first_name)} ${ssrInterpolate(asset.assigned_to_details.last_name)}</div></div></div>`);
        } else {
...
//# sourceMappingURL=index-ByYsy8AQ.js.map
