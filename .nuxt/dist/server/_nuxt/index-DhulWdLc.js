  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(filteredOrders), (po) => {
        ...
        _push(`</td><td class="px-6 py-4"><div class="font-medium">${ssrInterpolate(po.title)}</div><div class="text-xs text-muted-foreground">${ssrInterpolate(po.items?.length)} items</div></td><td class="px-6 py-4"><div class="font-medium">${ssrInterpolate(po.vendor?.name)}</div><div class="text-xs text-muted-foreground">${ssrInterpolate(po.vendor?.contact)}</div></td><td class="px-6 py-4 font-medium">₹${ssrInterpolate(po.totalAmount?.toLocaleString())}</td><td class="px-6 py-4"><div class="text-sm">${ssrInterpolate(formatDate(po.deliveryDate))}</div><div class="text-xs text-muted-foreground">${ssrInterpolate(po.deliveryLocation?.split(",")[0])}</div></td><td class="px-6 py-4"><span class="${ssrRenderClass([getStatusClass(po.status), "px-2 py-1 text-xs rounded-full font-medium"])}">${ssrInterpolate(formatStatus(po.status))}</span></td><td class="px-6 py-4 text-right"><div class="flex items-center justify-end gap-2">`);
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: `/admin/procurement/purchase-orders/${po.id}`,
...
//# sourceMappingURL=index-DhulWdLc.js.map
