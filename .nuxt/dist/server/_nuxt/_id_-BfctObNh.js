...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const formatRole = (role) => role?.split("_").map((s) => s.charAt(0).toUpperCase() + s.slice(1)).join(" ") || "";
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<span class="text-foreground">${ssrInterpolate(unref(user)?.full_name)}</span></nav><div class="flex items-start justify-between"><div class="flex items-center gap-4"><div class="w-20 h-20 bg-primary/10 rounded-2xl flex items-center justify-center"><span class="text-2xl font-bold text-primary">${ssrInterpolate(unref(user)?.first_name?.charAt(0))}${ssrInterpolate(unref(user)?.last_name?.charAt(0))}</span></div><div><h1 class="text-3xl font-bold">${ssrInterpolate(unref(user)?.full_name)}</h1><p class="text-muted-foreground">${ssrInterpolate(unref(user)?.email)}</p><div class="flex items-center gap-2 mt-2"><span class="${ssrRenderClass([getRoleClass(unref(user)?.role), "px-2 py-1 text-xs rounded-full"])}">${ssrInterpolate(formatRole(unref(user)?.role))}</span><span class="${ssrRenderClass([unref(user)?.is_active ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700", "px-2 py-1 text-xs rounded-full"])}">${ssrInterpolate(unref(user)?.is_active ? "Active" : "Inactive")}</span></div></div></div><div class="flex gap-2"><button class="flex items-center gap-2 px-4 py-2 border border-border rounded-lg hover:bg-muted">`);
      _push(ssrRenderComponent(_component_Icon, {
        name: "lucide:edit",
        class: "w-4 h-4"
      }, null, _parent));
      _push(` Edit </button>`);
      if (unref(user)?.is_active) {
      ...
      _push(`</div></div><div class="grid lg:grid-cols-3 gap-6"><div class="lg:col-span-2 space-y-6"><div class="bg-card border border-border rounded-xl p-6"><h2 class="text-lg font-semibold mb-4">Profile Information</h2><div class="grid md:grid-cols-2 gap-6"><div><label class="text-sm text-muted-foreground">First Name</label><p class="font-medium">${ssrInterpolate(unref(user)?.first_name)}</p></div><div><label class="text-sm text-muted-foreground">Last Name</label><p class="font-medium">${ssrInterpolate(unref(user)?.last_name)}</p></div><div><label class="text-sm text-muted-foreground">Email</label><p class="font-medium">${ssrInterpolate(unref(user)?.email)}</p></div><div><label class="text-sm text-muted-foreground">Phone</label><p class="font-medium">${ssrInterpolate(unref(user)?.phone || "Not provided")}</p></div><div><label class="text-sm text-muted-foreground">Department</label><p class="font-medium">${ssrInterpolate(unref(user)?.department || "Not assigned")}</p></div><div><label class="text-sm text-muted-foreground">Role</label><p class="font-medium">${ssrInterpolate(formatRole(unref(user)?.role))}</p></div></div></div><div class="bg-card border border-border rounded-xl p-6"><h2 class="text-lg font-semibold mb-4">Recent Activity</h2><div class="space-y-4"><div class="flex items-start gap-3 p-3 bg-muted/50 rounded-lg">`);
      _push(ssrRenderComponent(_component_Icon, {
        name: "lucide:log-in",
      ...
      _push(`<div><p class="font-medium">Approved requisition #PR-2024-015</p><p class="text-sm text-muted-foreground">2 days ago</p></div></div></div></div></div><div class="space-y-6"><div class="bg-card border border-border rounded-xl p-6"><h2 class="text-lg font-semibold mb-4">Account Stats</h2><div class="space-y-4"><div class="flex justify-between"><span class="text-muted-foreground">Member since</span><span class="font-medium">${ssrInterpolate(formatDate(unref(user)?.created_at))}</span></div><div class="flex justify-between"><span class="text-muted-foreground">Last updated</span><span class="font-medium">${ssrInterpolate(formatDate(unref(user)?.updated_at))}</span></div><div class="flex justify-between"><span class="text-muted-foreground">Tasks completed</span><span class="font-medium">24</span></div><div class="flex justify-between"><span class="text-muted-foreground">Approvals made</span><span class="font-medium">12</span></div></div></div><div class="bg-card border border-border rounded-xl p-6"><h2 class="text-lg font-semibold mb-4">Quick Actions</h2><div class="space-y-2"><button class="w-full py-2 px-3 text-left hover:bg-muted rounded-lg flex items-center gap-2">`);
      _push(ssrRenderComponent(_component_Icon, {
        name: "lucide:key",
        class: "w-4 h-4"
      }, null, _parent));
...
//# sourceMappingURL=_id_-BfctObNh.js.map
