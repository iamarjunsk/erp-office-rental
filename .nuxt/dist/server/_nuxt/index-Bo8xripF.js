  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(filteredUsers), (user) => {
        _push(`<tr class="hover:bg-muted/50"><td class="px-6 py-4"><div class="flex items-center gap-3"><div class="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center"><span class="text-sm font-medium text-primary">${ssrInterpolate(user.first_name?.charAt(0))}${ssrInterpolate(user.last_name?.charAt(0))}</span></div><div><p class="font-medium">${ssrInterpolate(user.full_name)}</p><p class="text-sm text-muted-foreground">${ssrInterpolate(user.email)}</p></div></div></td><td class="px-6 py-4"><span class="${ssrRenderClass([getRoleClass(user.role), "px-2 py-1 text-xs rounded-full"])}">${ssrInterpolate(formatRole(user.role))}</span></td><td class="px-6 py-4 text-muted-foreground">${ssrInterpolate(user.department || "-")}</td><td class="px-6 py-4"><span class="${ssrRenderClass([user.is_active ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700", "px-2 py-1 text-xs rounded-full"])}">${ssrInterpolate(user.is_active ? "Active" : "Inactive")}</span></td><td class="px-6 py-4 text-muted-foreground">${ssrInterpolate(formatDate(user.created_at))}</td><td class="px-6 py-4 text-right">`);
...
//# sourceMappingURL=index-Bo8xripF.js.map
