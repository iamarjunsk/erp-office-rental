...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div><div class="flex-1 min-w-0"><p class="text-sm font-semibold text-slate-900 truncate">${ssrInterpolate(((_a = unref(user)) == null ? void 0 : _a.first_name) || "User")} ${ssrInterpolate(((_b = unref(user)) == null ? void 0 : _b.last_name) || "")}</p><p class="text-xs text-slate-500 truncate group-hover:text-slate-700">${ssrInterpolate(((_c = unref(user)) == null ? void 0 : _c.email) || "Not logged in")}</p></div><button title="Logout" class="p-1 hover:bg-slate-100 rounded-lg transition-colors">`);
      _push(ssrRenderComponent(_component_Icon, {
        name: "lucide:log-out",
        class: "w-5 h-5 text-slate-400 hover:text-slate-900 transition-colors"
      }, null, _parent));
      _push(`</button></div></div></aside><main class="flex-1 md:ml-[280px] min-h-screen bg-slate-50/50"><header class="h-16 border-b border-border bg-background flex items-center justify-between px-8 sticky top-0 z-10"><h1 class="text-xl font-bold text-slate-800 capitalize">${ssrInterpolate(((_d = _ctx.$route.path.split("/").pop()) == null ? void 0 : _d.replace("-", " ")) || "Dashboard")}</h1><div class="flex items-center gap-4"><button class="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-colors relative">`);
      _push(ssrRenderComponent(_component_Icon, {
        name: "lucide:bell",
        class: "w-5 h-5"
      }, null, _parent));
...
//# sourceMappingURL=admin-DpkdtdYT.mjs.map
