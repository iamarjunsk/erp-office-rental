  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(invoices), (invoice) => {
        var _a2;
        _push(`<tr class="hover:bg-muted/50"><td class="px-4 py-3 text-sm font-medium">${ssrInterpolate(invoice.invoice_number)}</td><td class="px-4 py-3 text-sm">${ssrInterpolate((_a2 = invoice.company_details) == null ? void 0 : _a2.name)}</td><td class="px-4 py-3 text-sm">${ssrInterpolate(formatDate(invoice.period_start))} - ${ssrInterpolate(formatDate(invoice.period_end))}</td><td class="${ssrRenderClass([{ "text-red-500": isOverdue(invoice) }, "px-4 py-3 text-sm"])}">${ssrInterpolate(formatDate(invoice.due_date))}</td><td class="px-4 py-3 text-sm font-medium">${ssrInterpolate(formatCurrency(invoice.total_amount))}</td><td class="px-4 py-3 text-sm">${ssrInterpolate(formatCurrency(invoice.balance_due))}</td><td class="px-4 py-3 text-sm"><span class="${ssrRenderClass([{
...
//# sourceMappingURL=index-CzjHRmiz.mjs.map
