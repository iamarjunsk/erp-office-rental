  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(leases), (lease) => {
        var _a, _b;
        _push(`<tr class="hover:bg-muted/50"><td class="px-4 py-3 text-sm font-medium">${ssrInterpolate(lease.lease_number)}</td><td class="px-4 py-3 text-sm">${ssrInterpolate((_a = lease.company_details) == null ? void 0 : _a.name)}</td><td class="px-4 py-3 text-sm">${ssrInterpolate((_b = lease.space_details) == null ? void 0 : _b.code)}</td><td class="px-4 py-3 text-sm capitalize">${ssrInterpolate(lease.type.replace("_", " "))}</td><td class="px-4 py-3 text-sm">${ssrInterpolate(formatDate(lease.start_date))}</td><td class="px-4 py-3 text-sm">${ssrInterpolate(formatDate(lease.end_date))}</td><td class="px-4 py-3 text-sm">${ssrInterpolate(formatCurrency(lease.rent_amount))}</td><td class="px-4 py-3 text-sm"><span class="${ssrRenderClass([{
...
//# sourceMappingURL=index-Ck-1O1vS.mjs.map
