...
class MaintenanceRequestViewSet(viewsets.ModelViewSet):
    ...
    queryset = MaintenanceRequest.objects.all().select_related(
        ...
        django.db.models.Prefetch('tasks', queryset=MaintenanceTask.objects.select_related('assigned_to')),
        django.db.models.Prefetch('comments', queryset=MaintenanceComment.objects.select_related('author'))
    ).order_by('-created_at')
    ...
    def stats(self, request):
        ...
        # ⚡ Bolt: Consolidated 7 separate .count() queries into a single .aggregate() call using Count and Q expressions.
        # This reduces database roundtrips and significantly improves API latency.
        stats = MaintenanceRequest.objects.aggregate(
            ...
            high=Count('id', filter=Q(priority='high', status__in=['open', 'assigned', 'in_progress'])),
            overdue=Count('id', filter=Q(scheduled_date__lt=timezone.now().date(), status__in=['open', 'assigned', 'in_progress']))
        )
