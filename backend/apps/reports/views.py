...
class DashboardStatsView(APIView):
    ...
    def get_stats(self):
        ...
        space_stats = Space.objects.aggregate(
            total=Count('id'),
            occupied=Count('id', filter=Q(current_status='occupied'))
        )
        ...
        # Maintenance
        ticket_stats = MaintenanceRequest.objects.aggregate(
            total=Count('id'),
            completed=Count('id', filter=Q(status='completed'))
        )
        total_tickets = ticket_stats['total']
        completed_tickets = ticket_stats['completed']

        # Leases
        lease_stats = Lease.objects.filter(status='active').aggregate(
            active=Count('id'),
            expiring=Count('id', filter=Q(end_date__lte=today + timedelta(days=30)))
        )
